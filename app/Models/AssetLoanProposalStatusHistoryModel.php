...
class AssetLoanProposalStatusHistoryModel extends Model
{
    ...
    public function getApprovalHistory(int $perPage, ?int $changedBy, string $search = '', string $status = '', string $laboratoryUuid = ''): array
    {
        ...
        if ($laboratoryUuid !== '') {
            $query->join('asset_loan_proposal_items', 'asset_loan_proposal_items.proposal_id = asset_loan_proposal_status_histories.proposal_id')
                ...
                ->groupBy('asset_loan_proposal_status_histories.id');
        }

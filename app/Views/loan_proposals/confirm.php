...
<div class="page__section flex flex-col gap-4">
  <div class="card">
    ...
    <div class="card__body">
      <p class="text-sm text-muted-foreground" style="margin:0 0 1rem;">Periksa kembali data pengajuan dan ruangan yang akan dipinjam sebelum mengajukan.</p>
  ...
  <div class="loan-confirm__grid">
    ...
    <div class="card">
      <div class="card__body">
        ...
        <button type="button" class="button button--primary w-full" onclick="openLoanConfirmDialog()">
            Ajukan Peminjaman
...
<div class="dialog dialog--sm" id="loanSubmitConfirm" data-stisla-dialog data-state="closed" role="alertdialog" aria-modal="true" aria-labelledby="loanSubmitConfirmLabel" aria-describedby="loanSubmitConfirmDesc" aria-hidden="true" tabindex="-1">
  ...
  <div class="dialog__panel">
    <div class="dialog__content">
      ...
      <div class="dialog__body text-center pt-6">
        ...
        <h3 class="dialog__title mb-1" id="loanSubmitConfirmLabel">Ajukan peminjaman ini?</h3>
        <p class="text-muted-foreground" id="loanSubmitConfirmDesc">Data pengajuan akan dikirim untuk diproses setelah Anda mengonfirmasi.</p>
      </div>
      <form id="loanSubmitForm" action="<?= base_url('peminjaman/lab-loans/submit/' . $proposal['uuid']) ?>" method="post">
        <?= csrf_field() ?>
        <div class="dialog__footer justify-center">
          ...
          <button type="submit" class="button button--primary">Ya, Ajukan</button>
        </div>
      </form>
    </div>
  </div>
...
<script>
  function openLoanConfirmDialog() {
    var dialog = document.getElementById('loanSubmitConfirm');
    if (!dialog) return;
  ...
  document.querySelector('#loanSubmitForm').addEventListener('submit', function () {

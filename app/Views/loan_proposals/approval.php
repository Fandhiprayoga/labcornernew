  ...
  <div class="card">
    <div class="card__header">
      ...
      <div class="card__action">
        <a href="<?= base_url('peminjaman/lab-loans') ?>" class="button button--outline button--neutral button--sm">Daftar Pengajuan</a>
      </div>
    </div>
    <div class="card__body" style="border-bottom:1px solid var(--color-border);">
      <div class="toggle-group" data-stisla-toggle-group role="radiogroup" aria-label="Jenis data approval" style="display:inline-flex;width:max-content;max-width:100%;overflow-x:auto;">
        <a href="<?= base_url('peminjaman/lab-loans-approval?tab=pending') ?>" class="toggle" role="radio" aria-checked="<?= $tab === 'pending' ? 'true' : 'false' ?>" data-state="<?= $tab === 'pending' ? 'active' : 'inactive' ?>">Pengajuan Butuh Approval</a>
      ...
      <div class="table-responsive">
        <table class="table">
          ...
          <?php if (! empty($proposals)): ?>
            ...
            <tr>
              ...
              <td class="text-end">
                <div class="flex justify-end gap-1">
                  <a href="<?= base_url('peminjaman/lab-loans/detail-approval/' . $proposal['uuid']) ?>" class="button button--info button--icon-only button--sm" title="Detail Pengajuan" aria-label="Detail Pengajuan">
            ...
            <?php endforeach; ?>
          <?php else: ?>
            <tr><td colspan="5" class="text-center text-muted-foreground py-8"><?= view('partials/empty_table_state', ['message' => 'Tidak ada pengajuan yang menunggu approval pada tahap ini.']) ?></td></tr>
          <?php endif; ?>
          </tbody>
        </table>
      </div>
    ...
    <?php if ($pager->getTotal() > 0): ?><div class="card__body" style="border-top:1px solid var(--color-border);display:flex;flex-wrap:wrap;align-items:center;justify-content:space-between;gap:.75rem;"><span class="text-xs text-muted-foreground">Total <?= $pager->getTotal() ?> <?= $tab === 'history' ? 'history approval' : 'pengajuan' ?></span><?= $pager->only(['tab', 'q', 'status', 'laboratory_uuid', 'perPage'])->links('default', 'app') ?></div><?php endif; ?>
  </div>
</div>
  ...
  <div class="dialog__panel">
    <div class="dialog__content">
      ...
      <div class="dialog__body text-center pt-6">
        ...
        <h3 class="dialog__title mb-1" id="approveConfirmLabel">Setujui pengajuan ini?</h3>
        <p class="text-muted-foreground" id="approveConfirmDesc">Pengajuan <strong data-slot="approve-event"></strong> akan lanjut ke tahap persetujuan berikutnya.</p>
      </div>
  ...
  <div class="dialog__panel">
    <div class="dialog__content">
      <div class="dialog__header">
        <h3 class="dialog__title" id="rejectConfirmLabel">Tolak Pengajuan</h3>
        <button type="button" class="dialog__close" data-stisla-dialog-dismiss aria-label="Tutup">
          <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" aria-hidden="true"><path d="M6 6l12 12M18 6L6 18" /></svg>
        </button>
      </div>
      <form id="rejectForm" method="post">
        ...
        <div class="dialog__body">
          <p class="text-muted-foreground text-sm mb-4">Anda akan menolak pengajuan <strong data-slot="reject-event"></strong>. Alasan ini akan terlihat oleh pemohon dan tahap approval sebelumnya.</p>
          <div class="field">
            <label class="field__label" for="reject_note">Alasan Penolakan <span class="text-danger">*</span></label>
            <textarea class="input" id="reject_note" name="note" rows="3" required placeholder="Tuliskan alasan penolakan pengajuan ini..."></textarea>
          </div>
        </div>

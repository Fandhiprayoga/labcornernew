...
<div class="dialog dialog--sm" id="roomDeleteConfirm" data-state="closed" role="alertdialog" aria-modal="true" aria-labelledby="roomDeleteConfirmLabel" aria-describedby="roomDeleteConfirmDesc" aria-hidden="true" tabindex="-1">
  <div class="dialog__backdrop" data-room-dialog-dismiss></div>
  <div class="dialog__panel">
    <div class="dialog__content">
      <button type="button" class="dialog__close" data-room-dialog-dismiss aria-label="Tutup"><svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" aria-hidden="true"><path d="M6 6l12 12M18 6L6 18" /></svg></button>
      <div class="dialog__body text-center pt-6">
        ...
        <h3 class="dialog__title mb-1" id="roomDeleteConfirmLabel">Hapus ruangan?</h3>
        <p class="text-muted-foreground" id="roomDeleteConfirmDesc">Ruangan <strong id="roomDeleteConfirmName"></strong> akan dihapus.</p>
      </div>
      <form id="roomDeleteForm" method="post">
        <?= csrf_field() ?>
        <div class="dialog__footer justify-center">
          <button type="button" class="button button--outline button--neutral" data-room-dialog-dismiss>Batal</button>
...
<script>
  document.addEventListener('DOMContentLoaded', function () {
    var dialog = document.getElementById('roomDeleteConfirm');
    var modalForm = document.getElementById('roomDeleteForm');
    var name = document.getElementById('roomDeleteConfirmName');
    ...
    document.querySelectorAll('[data-room-delete-form]').forEach(function (form) {
      form.addEventListener('submit', function (event) {
        ...
        window.requestAnimationFrame(function () { modalForm.querySelector('[data-room-dialog-dismiss]').focus(); });
      });
    });

    dialog.querySelectorAll('[data-room-dialog-dismiss]').forEach(function (element) {

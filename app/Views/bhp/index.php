...
<div class="page__section flex flex-col gap-4">
  <div class="card">
    <div class="card__header">
      <div><span class="card__title">Pengajuan BHP Program Studi</span>
        <div class="text-xs text-muted-foreground">Pilih program studi untuk menambahkan item ke pengajuan BHP.</div>
      </div>
    </div>
    <div class="card__body">
      <form method="get" action="<?= base_url('bhp') ?>" style="display:flex;flex-wrap:wrap;align-items:flex-end;gap:.75rem;">
        ...
        <div style="flex:0 1 190px;min-width:170px;"><label class="text-xs text-muted-foreground" for="status">Status</label><select class="select" id="status" name="status">
        ...
        <table class="table">
          ...
          <tbody>
            ...
            <?php foreach ($requests as $row): ?>
              ...
              <tr>
                <td><strong><?= esc($row['kode_pengajuan']) ?></strong>
                  <div class="text-xs text-muted-foreground"><?= esc($row['uuid']) ?></div>
                </td>
                <td><?= esc($row['study_program_name'] ?? $row['prodi_snapshot'] ?? '-') ?></td>
                <td><?= esc($row['nama_periode']) ?><div class="text-xs text-muted-foreground"><?= esc($periodRange) ?></div>
                </td>
                  ...
                  <div class="flex justify-end gap-1">
                    <?php if (activeGroupCan('bhp.create') && in_array($row['status'], ['DRAFT', 'NEED_REVISION'], true)): ?>
                      <?php if ($periodAllowsModification): ?>
                        <a class="button button--primary button--icon-only button--sm" href="<?= base_url('bhp/create?pocket_uuid=' . $row['uuid']) ?>" title="Tambah item" aria-label="Tambah item"><svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24" aria-hidden="true">
                            <path fill="none" stroke="currentColor" stroke-linecap="round" stroke-width="1.5" d="M12 5v14m-7-7h14" />
                          </svg></a>
                      <?php else: ?>
                        <button type="button" class="button button--primary button--icon-only button--sm" title="Periode pengajuan sudah ditutup" aria-label="Periode pengajuan sudah ditutup" disabled><svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24" aria-hidden="true">
                            <path fill="none" stroke="currentColor" stroke-linecap="round" stroke-width="1.5" d="M12 5v14m-7-7h14" />
                          </svg></button>
                      <?php endif; ?>
                    <?php endif; ?>
                    <?php if (in_array($row['status'], ['DRAFT', 'NEED_REVISION'], true) && activeGroupCan('bhp.edit')): ?>
                      <?php if (! $periodAllowsModification): ?>
                        <button type="button" class="button button--warning button--icon-only button--sm" title="Periode pengajuan sudah ditutup" aria-label="Periode pengajuan sudah ditutup" disabled><svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24" aria-hidden="true">
                      ...
                      <?php elseif ($editableItemCount > 0): ?>
                        <a class="button button--warning button--icon-only button--sm" href="<?= base_url('bhp/edit/' . $row['uuid']) ?>" title="Edit" aria-label="Edit pengajuan"><svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24" aria-hidden="true">
                            <path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="m16.5 4.5 3 3L8 19H5v-3L16.5 4.5Z" />
                          </svg></a>
                      <?php else: ?>
                        <button type="button" class="button button--warning button--icon-only button--sm" title="Belum ada item untuk diedit" aria-label="Belum ada item untuk diedit" disabled><svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24" aria-hidden="true">
                            <path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="m16.5 4.5 3 3L8 19H5v-3L16.5 4.5Z" />
                          </svg></button>
                      <?php endif; ?>
                    <?php endif; ?>
                    <a class="button button--info button--icon-only button--sm" href="<?= base_url('bhp/detail/' . $row['uuid']) ?>" title="Detail" aria-label="Lihat detail pengajuan"><svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24" aria-hidden="true">
                        <path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M2.5 12s3.5-6 9.5-6 9.5 6-9.5 6-9.5-6-9.5-6Z" />
